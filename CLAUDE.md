...
- `GetStateService` (`/GetState.csv`, GET) — the polling engine. `start(successCb?, errorCb?, stopOnError?)` kicks off a self-rescheduling `setTimeout` loop driven by `updateInterval`. Tracks `_consecutiveFails` against `errorTolerance` (constructor-clamped to `>=1`); only invokes the error callback once the limit is hit. **Requests are serialised**: `autoUpdate()` schedules the next tick from `.finally()`, so a slow request can't trigger overlapping in-flight fetches — effective interval becomes `max(updateInterval, request_time)`. The `_polling` flag tracks the loop state; `isRunning()` reports it.
...
- `CommandService` (`/Command.htm`, GET) — manual dosage (`MAN_DOSAGE=<target>,<seconds>`). Targets: chlorine / pH-minus / pH-plus. Uses `this.request({ params: { MAN_DOSAGE: '<target>,<seconds>' } })` so the central auth / timeout / typed-error path is shared with the other services; retries up to 3× internally before returning `-1`. Non-finite durations throw `ProconIpError` immediately.
- `SetStateService` (`/SetState.pl`, GET) — generic relay on-timer. Same `this.request({ params })` pattern. Public API takes seconds; the controller wants ms — we `Math.trunc` to integer seconds first then multiply, so floating-point artefacts (e.g. `0.3 * 1000` → `300.00000000000006`) never land in the raw-concat query string.
